...
namespace LibraryEditor
{
    ...
    public class WTLImage
    {
        ...
        private static int Unpack(IList<byte> packed, int srcOffset, IList<byte> colour, int dstOffSet)
        {
            ...
            // get components in the stored range   5 6 5
        ...
        private static int Pack(IList<byte> packed, int srcOffset, IList<byte> colour, int dstOffSet)
        {
            byte blue = colour[dstOffSet + 0];
            byte green = colour[dstOffSet + 1];
            byte red = colour[dstOffSet + 2];

            blue = (byte)(blue >> 3);
            green = (byte)(blue >> 2);
            red = (byte)(blue >> 3);

            int value = (red << 11) | (green << 5) | blue;
            packed[srcOffset] = (byte)(value & 0xF);
            packed[srcOffset + 1] = (byte)(value >> 8);
            //*/
            return value;
        }
    }
