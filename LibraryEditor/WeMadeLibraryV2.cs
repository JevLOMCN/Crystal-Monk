...
namespace LibraryEditor
{
    public class WeMadeLibraryV2
    {
        ...
        public WeMadeLibraryV2(string name)
        {
        ...
        public void ToMLibrary(ToolStripProgressBar toolStripProgressBar)
        {
           ...
           // if (toolStripProgressBar.Par.InvokeRequired)
           // {
             //   toolStripProgressBar.Parent.Invoke(new MethodInvoker(delegate { toolStripProgressBar.Maximum = Images.Length; }));
           // }
            //   toolStripProgressBar.Maximum = Images.Length;

            int Count = Images.Length;
            FileStream _stream = File.Create(fileName);
            BinaryWriter writer = new BinaryWriter(_stream);
            writer.Write(MLibraryV2.LibVersion);
            writer.Write(Count);
            ...
            try
            {
                for (int i = 0; i < Count; i++)
                {
                    ...
                    writer.Seek(8 + 4 * i, SeekOrigin.Begin);
                    writer.Write((int)_stream.Length);

                    WeMadeImage image = Images[i];
                    MLibraryV2.MImage temp;
                    if (image.HasMask)
                        temp = new MLibraryV2.MImage(image.Image, image.MaskImage) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.boHasShadow ? (byte)1 : (byte)0, MaskX = image.X, MaskY = image.Y };
                    else
                        temp = new MLibraryV2.MImage(image.Image) { X = image.X, Y = image.Y, ShadowX = image.ShadowX, ShadowY = image.ShadowY, Shadow = image.boHasShadow ? (byte)1 : (byte)0 };
            ...
            MessageBox.Show(string.Format("Successfully converted {0}", _fileName));
            // Operation finished.
        ...
        {
            string _newName = Path.GetFileName(_fileName) ;
            ...
            for (int i=0; i<Images.Length; ++i)
            {
                ...
                Images[i].Image.Save(_folder + _newName + i + ".bmp");
            }
        }
        ...
        public class WeMadeImage
        {
            ...
            public unsafe void CreateTexture(BinaryReader reader, int[] palette)
            {
                ...
                if (bo16bit)
                {
                    BitmapData data = Image.LockBits(new Rectangle(0, 0, Width, Height), ImageLockMode.WriteOnly, PixelFormat.Format16bppRgb565);
                    MemoryStream output;
                    switch (nType)
                    {
                    ...
                    ushort* scan0 = (ushort*)data.Scan0;
                    {
                        for (int y = Height - 1; y >= 0; y--)
                        {
                            for (int x = 0; x < Width; x++)
                            {
                                 scan0[y * Width + x] = (ushort)(bytes[index++] + (bytes[index++] << 8));
                            }
                        }
                    }
