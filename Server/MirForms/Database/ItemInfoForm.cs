    ...
    public partial class ItemInfoForm : Form
    {
        ...
        public void UpdateInterface(bool refreshList = false)
        {
            if (refreshList)
            {
                ...
                List<ItemInfo> list = Envir.GetItemInfoList();
                foreach (var itemInfo in list) 
                {
                    if (!string.IsNullOrEmpty(SearchTextBox.Text) && !Envir.Tr(itemInfo.Name).Contains(SearchTextBox.Text))
                        continue;

                    if (ITypeFilterComboBox.SelectedItem == null 
                        || ITypeFilterComboBox.SelectedIndex == ITypeFilterComboBox.Items.Count - 1 
                        || Envir.Tr(itemInfo.Type.ToString()) == (string)ITypeFilterComboBox.SelectedItem)
                        ItemInfoListBox.Items.Add(itemInfo);
                }
            }
            ...
            MinSockNumTextBox.Text = info.MinSocket.ToString();
            MaxSockNumTextBox.Text = info.MaxSocket.ToString(); 

            LevelBasedcheckbox.Checked = info.LevelBased;
            ClassBasedcheckbox.Checked = info.ClassBased;
            HumUpCheckBox.Checked = info.NeedHumUp;
            ...
            for (int i = 1; i < _selectedItemInfos.Count; i++)
            {
                ...
                if (HumUpCheckBox.Checked != info.NeedHumUp) HumUpCheckBox.CheckState = CheckState.Indeterminate;
        ...
        private void RefreshItemList()
        {
            ...
            List<ItemInfo> itemInfoList = Envir.GetItemInfoList();
            for (int i = 0; i < itemInfoList.Count; i++)
            {
                if (ITypeFilterComboBox.SelectedItem == null ||
                    ITypeFilterComboBox.SelectedIndex == ITypeFilterComboBox.Items.Count - 1 ||
                    Envir.Tr(itemInfoList[i].Type.ToString()) == (string)ITypeFilterComboBox.SelectedItem)
                    ItemInfoListBox.Items.Add(itemInfoList[i]);
            };
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            {
                ItemType type = (ItemType)Envir.String2Enum((string)ITypeFilterComboBox.SelectedItem, typeof(ItemType));
        ...
        private void PasteButton_Click(object sender, EventArgs e)
        {
            ...
            ItemInfo info = ItemInfo.FromText(_selectedItemInfos[0].ToText());

            info.Index = ++Envir.ItemIndex;
            info.Name += info.Index;
            Envir.ItemInfoList.Add(info.Index, info);
        ...
        private void CopyMButton_Click(object sender, EventArgs e)
        {
            ...
            Clipboard.SetText(ItemInfo.ToHeader() + "\r\n" + _selectedItemInfos[0].ToText());
        }

        private void ExportAllButton_Click(object sender, EventArgs e)
        {
            ExportItems(Envir.GetItemInfoList());
        }
        ...
        private void ImportButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 1; i < items.Length; i++)
                {
                    ...
                    {
                        MessageBox.Show(string.Format("第{0}行 {1}", i, items[i]));
        ...
        private void MaxSockNumTextBox_TextChanged(object sender, EventArgs e)
        {
            byte temp;
            if (ActiveControl != sender)
                return;
        ...
        private void MinSockNumTextBox_TextChanged(object sender, EventArgs e)
        {
            byte temp;
            if (ActiveControl != sender)
                return;
        ...
        private void checkBoxHumUp_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;

            for (int i = 0; i < _selectedItemInfos.Count; i++)
                _selectedItemInfos[i].NeedHumUp = HumUpCheckBox.Checked;
        }
    }
}
