...
namespace Server
{
    public partial class GameShop : Form
    {
        ...
        {
             ExportItems(Envir.GameShopList);
        }

        private void ExportItems(IEnumerable<GameShopItem> items)
        {
            var utf8WithBom = new UTF8Encoding(true);
            var itemInfos = items as GameShopItem[] ?? items.ToArray();
            var list = itemInfos.Select(item => item.ToText()).ToList();
            // File.WriteAllLines(ItemListPath, list);
            string path = Path.Combine(Settings.ExportPath, "GameShopItem.txt");
            using (StreamWriter file = new StreamWriter(path, false, utf8WithBom))
            {
                file.WriteLine(GameShopItem.ToHeader());
                foreach (GameShopItem item in itemInfos)
                    file.WriteLine(item.ToText());// 直接追加文件末尾，换行 
            }

            MessageBox.Show(" Items have been exported");
        }
