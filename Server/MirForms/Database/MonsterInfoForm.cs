...
namespace Server
{
    public partial class MonsterInfoForm : Form
    {
        ...
        public MonsterInfoForm()
        {
            ...
            BTypeComboBox.Items.AddRange(Enum.GetValues(typeof(MonsterType)).Cast<object>().ToArray());
        ...
        private void UpdateInterface(bool refresh = false)
        {
            if (refresh)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(SearchTextBox.Text) && !Envir.Tr(Envir.MonsterInfoList[i].Name).Contains(SearchTextBox.Text))
                        continue;
            ...
            if (_selectedMonsterInfos.Count == 0)
            {
                ...
                BTypeComboBox.SelectedItem = null;
            ...
            BTypeComboBox.SelectedItem = info.BType;
            ...
            for (int i = 1; i < _selectedMonsterInfos.Count; i++)
            {
                ...
                if (BTypeComboBox.SelectedItem == null || (MonsterType)BTypeComboBox.SelectedItem != info.BType) BTypeComboBox.SelectedItem = null;
        ...
        private void PoisonResistTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _selectedMonsterInfos.Count; i++)
                _selectedMonsterInfos[i].PoisonResist = temp;
        }
        ...
        private void ButtonBoss_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Envir.MonsterInfoList.Count; i++)
            {
                if (Envir.MonsterInfoList[i].Level < 66)
                    continue;
        ...
        private void BTypeComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;

            for (int i = 0; i < _selectedMonsterInfos.Count; i++)
                _selectedMonsterInfos[i].BType = (MonsterType)BTypeComboBox.SelectedItem;
        }
    }
}