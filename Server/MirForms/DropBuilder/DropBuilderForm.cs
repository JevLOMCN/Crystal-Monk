...
namespace Server.MirForms.DropBuilder
{
    public partial class DropGenForm : Form
    {
        ...
        private void tabControlSeperateItems_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (var itemInfo in Envir.ItemInfoList.Values)
            {
                if (itemInfo.Type.ToString() == Tab.SelectedTab.Tag.ToString())
                {
                    try
                    {
                        if (textBoxMinLevel.Text == string.Empty || textBoxMaxLevel.Text == string.Empty)                            
                            TempListBox.Items.Add(itemInfo.Name);
                        else if (itemInfo.RequiredAmount >= int.Parse(textBoxMinLevel.Text) &
                            itemInfo.RequiredAmount <= int.Parse(textBoxMaxLevel.Text))
                            TempListBox.Items.Add(itemInfo.Name);
                    }
                    catch (Exception)
                    {
