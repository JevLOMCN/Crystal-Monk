...
namespace Server.MirNetwork
{
    ...
    public class NetStat
    {
        ...
        public int RecvNumPerSec;
        public long LastStatSec;
    }
    ...
    public class MirConnection
    {
        ...
        public NetStat NetStat = new NetStat();
        ...
        private void ReceiveData(IAsyncResult result)
        {
            ...
            try
            {
                ...
                ServiceNetStat.AddStat(NetStat, dataRead);
                ...
                while (true)
                {
                    p = Packet.ReceivePacket(_rawData, out _rawData, ref retcode);
                    if (retcode != 0)
                    {
                        SMain.Enqueue(string.Format("非法IP {0}. ", IPAddress));
                        Disconnecting = true;
                        return;
                    }
        ...
        public void Process()
        {
            ...
            while (!_receiveList.IsEmpty && !Disconnecting)
            {
                ...
                {
                    SMain.Enqueue(string.Format("Process {0}", ex.ToString()));
                }
            }
    
            if (_receiveList.Count > 100)
                SMain.Enqueue(string.Format("_receive list {0}", _receiveList.Count));
           ...
           // {
            ...
            while (!_sendList.IsEmpty)
            {
                ...
                IEnumerable<byte> bytes = p.GetPacketBytes();
                data.AddRange(bytes);

            //    SMain.EnqueueDebugging(string.Format("send {0} len {1}", p.Index, bytes.Count()));
            }
        ...
        private void ProcessPacket(Packet p)
        {
            ...
            switch (p.Index)
            {
                ...
                case (short)ClientPacketIds.EERemoveRune:
                    RemoveRune((C.RemoveRune)p);
                    break;
                case (short)ClientPacketIds.DownloadFile:
                    DownLoadFile((C.DownloadFile)p);
                    break;
                case (short)ClientPacketIds.ChangeNameReturn:
                    ChangeNameReturn((C.ChangeNameReturn)p);
                    break;
                default:
                    Disconnecting = true;
                    SMain.Enqueue(string.Format("Invalid packet received. Index : {0} IP:{1}", p.Index, IPAddress));
                    break;
            }
        }

        private void ChangeNameReturn(C.ChangeNameReturn p)
        {
            if (Stage != GameStage.Game) return;
        ...
        private void DeleteCharacter(C.DeleteCharacter p)
        {
            ...
            SMain.Envir.RemoveRank(Account.RegionID, temp);
        ...
        private void ConsignItem(C.ConsignItem p)
        {
            ...
            Player.ConsignItem(p.UniqueID, p.Price, p.CurrencyType);
        }
        private void MarketSearch(C.MarketSearch p)
        {
            ...
            MarketMatcher matcher = Player.MarketMatcher;
            matcher.MinShapes = p.MinShape;
            matcher.MaxShapes = p.MaxShape;
            matcher.MatchName = p.Match;
            matcher.MatchType = p.Type;
            matcher.GradeType = p.Grade;
            matcher.RequiredClass = p.RequiredClass;
            Player.MarketSearch(matcher);
        }
        ...
        private void Awakening(C.Awakening p)
        {
            ...
            Player.Awakening(p.UniqueID, p.Type, p.MaterialUniqueID);
        }
        ...
        private void ReportIssue(C.ReportIssue p)
        {
            ...
            if (p.ImageSize > 0 && p.ImageChunk >= p.ImageSize)
            {
       ...
       //     SMain.EnqueueDebugging(p.Message);
        }
        ...
        private void SocketItem(C.SocketRuneStone p)
        {
            if (Stage != GameStage.Game)
                return;
        ...
        private void RemoveRune(C.RemoveRune p)
        {
            if (Stage != GameStage.Game)
                return;
        }

        private void DownLoadFile(C.DownloadFile p)
        {
            if (Stage != GameStage.Game)
                return;

            Player.DownloadFile(p.FileName, (int)p.Start);
        }
    }
}
