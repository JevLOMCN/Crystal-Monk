...
namespace Server.MirDatabase
{
    public class QuestProgressInfo
    {
        ...
        public bool ContainItem(ItemInfo iInfo)
        {
            return Info.ItemTasks.Where((task, i) => task.Item == iInfo).Any();
        }

        public bool NeedKill(MonsterInfo mInfo)
        {
            return Info.KillTasks.Where((task, i) => KillTaskCount[i] < task.Count && mInfo.GameName.Equals(task.Monster.GameName, StringComparison.OrdinalIgnoreCase)).Any();
        }
        ...
        public void ProcessKill(MonsterInfo mInfo)
        {
            ...
            for (int i = 0; i < Info.KillTasks.Count; i++)
            {
                if (!mInfo.GameName.Equals(Info.KillTasks[i].Monster.GameName, StringComparison.OrdinalIgnoreCase)) continue;
                KillTaskCount[i]++;
        ...
        public bool ProcessItem(UserItem[] inventory)
        {
            bool changed = false;
            for (int i = 0; i < Info.ItemTasks.Count; i++)
            {
        ...
        public void UpdateKillTasks()
        {
            if(Info.KillMessage.Length > 0 && Info.KillTasks.Count > 0) 
            {
                ...
                TaskList.Add(string.Format("{0} {1}", Info.KillMessage, allComplete ? Envir.Tr("(Completed)") : ""));
                return;
            }
            ...
            for (int i = 0; i < Info.KillTasks.Count; i++)
            {
                if (string.IsNullOrEmpty(Info.KillTasks[i].Message))
                    TaskList.Add(string.Format(Envir.Tr("Kill {0}: {1}/{2} {3}"), Info.KillTasks[i].Monster.GameName, KillTaskCount[i],
                        Info.KillTasks[i].Count, KillTaskCount[i] >= Info.KillTasks[i].Count ? Envir.Tr("(Completed)") : ""));
                else
                    TaskList.Add(string.Format("{0} {1}", Info.KillTasks[i].Message, KillTaskCount[i] >= Info.KillTasks[i].Count ? Envir.Tr("(Completed)") : ""));
        ...
        public void UpdateItemTasks()
        {
            if (Info.ItemMessage.Length > 0 && Info.ItemTasks.Count > 0)
            {
                ...
                TaskList.Add(string.Format("{0} {1}", Info.ItemMessage, allComplete ? Envir.Tr("(Completed)") : ""));
                return;
            }
            ...
            for (int i = 0; i < Info.ItemTasks.Count; i++)
            {
                if (string.IsNullOrEmpty(Info.ItemTasks[i].Message))
                    TaskList.Add(string.Format(Envir.Tr("Collect {0}: {1}/{2} {3}"), Info.ItemTasks[i].Item.Name, ItemTaskCount[i],
                        Info.ItemTasks[i].Count, ItemTaskCount[i] >= Info.ItemTasks[i].Count ? Envir.Tr("(Completed)") : ""));
                else
                    TaskList.Add(string.Format("{0} {1}", Info.ItemTasks[i].Message, ItemTaskCount[i] >= Info.ItemTasks[i].Count ? Envir.Tr("(Completed)") : ""));
            }
        }
        ...
        public void UpdateFlagTasks()
        {
            if (Info.FlagMessage.Length > 0)
            {
                ...
                TaskList.Add(string.Format("{0} {1}", Info.FlagMessage, allComplete ? Envir.Tr("(Completed)") : ""));
                return;
            }
            ...
            for (int i = 0; i < Info.FlagTasks.Count; i++)
            {
                if (string.IsNullOrEmpty(Info.FlagTasks[i].Message))
                    TaskList.Add(string.Format(Envir.Tr("Activate Flag {0} {1}"), Info.FlagTasks[i].Number, FlagTaskSet[i] ? Envir.Tr("(Completed)") : ""));
                else
                    TaskList.Add(string.Format("{0} {1}", Info.FlagTasks[i].Message, FlagTaskSet[i] ? Envir.Tr("(Completed)") : ""));
