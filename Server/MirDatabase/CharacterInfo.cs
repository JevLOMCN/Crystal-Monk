...
namespace Server.MirDatabase
{
    public class CharacterInfo
    {
        ...
        public UserItem[] Inventory = new UserItem[46], Equipment = new UserItem[Enum.GetNames(typeof(EquipmentSlot)).Length], Trade = new UserItem[10], Refine = new UserItem[16];
        ...
        public CharacterInfo(BinaryReader reader)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (SMain.Envir.BindItem(item) && i < Inventory.Length)
                {
                    ...
                    if (Inventory[i].Info.Type == ItemType.Awakening && (SpecialItemShape)Inventory[i].Info.Shape == SpecialItemShape.SuccessRateUpItem && Inventory[i].Count > 10000)
                    {
                        SMain.Enqueue(string.Format("觉醒材料 {0}", Name));
                    }
                }
            }
            ...
            if(Envir.LoadVersion > 43)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    MailInfo info = new MailInfo(reader, Envir.LoadVersion, Envir.LoadCustomVersion);
                    if (info.MailID == 0)
                    {
                        info.MailID = SMain.Envir.GenerateMailId();
                        SMain.EnqueueDebugging(string.Format("MailInfo restore {0}", Name));
                    }
                    Mail.Add(info);
                }
            }
            ...
            if (Envir.LoadVersion > 82)
            {
                int pairCount = reader.ReadInt32();
                for (int i = 0; i < pairCount; i++)
                {
                    NPCVar.Add( new KeyValuePair<string, string>(reader.ReadString(), reader.ReadString()) );
                }
            }
        ...
        public void Save(BinaryWriter writer)
        {
            ...
            writer.Write(NPCVar.Count);
            for (int i = 0; i < NPCVar.Count; i++)
            {
                writer.Write(NPCVar[i].Key);
                writer.Write(NPCVar[i].Value);
            }
        ...
        public uint GetInventoryItemCount(int itemId)
        {
            uint count = 0;
            for (int i=0; i< Inventory.Length; ++i)
            {
                if (Inventory[i].ItemIndex != itemId)
                    continue;
    ...
    public class IntelligentCreatureInfo
    {
        ...
        public string Info = "";
        public string Info1 = "不能制造灵物石.";
        //public string Info2 = "Can produce Pearls, used to buy Creature items.";
        // TODO 翻译
        public string Info2 = "可以生产珍宝，用于购买宠物道具";

        static IntelligentCreatureInfo()
        {
            BabyPig = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.BabyPig, Icon = 500, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 3, MinimalFullness = 4000, Info = Envir.Tr("Can pickup items (3x3 semi-auto).") };
            Chick = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.Chick, Icon = 501, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 7, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 7, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (7x7 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.") };
            Kitten = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.Kitten, Icon = 502, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 3, MinimalFullness = 6000, Info = Envir.Tr("Can pickup items (5x5 semi-auto).") };
            BabySkeleton = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.BabySkeleton, Icon = 503, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 7, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 7, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (7x7 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.") };
            Baekdon = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.Baekdon, Icon = 504, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 7, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 7, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (7x7 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.") };
            Wimaen = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.Wimaen, Icon = 505, MousePickupEnabled = true, MousePickupRange = 7, AutoPickupEnabled = true, AutoPickupRange = 5, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 5, MinimalFullness = 5000,
                Info = Envir.Tr("Can pickup items (5x5 auto/semi-auto, 7x7 mouse).") };
            BlackKitten = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.BlackKitten, Icon = 506, MousePickupEnabled = true, MousePickupRange = 7, AutoPickupEnabled = true, AutoPickupRange = 5, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 5, MinimalFullness = 5000,
                Info = Envir.Tr("Can pickup items (5x5 auto/semi-auto, 7x7 mouse).") };
            BabyDragon = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.BabyDragon, Icon = 507, MousePickupEnabled = true, MousePickupRange = 7, AutoPickupEnabled = true, AutoPickupRange = 5, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 5, MinimalFullness = 7000,
                Info = Envir.Tr("Can pickup items (5x5 auto/semi-auto, 7x7 mouse).") };
            OlympicFlame = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.OlympicFlame, Icon = 508, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 11, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 11, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (11x11 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.") };
            BabySnowMan = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.BabySnowMan, Icon = 509, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 11, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 11, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (11x11 auto/semi-auto, 11x11 mouse)."), Info1 =  Envir.Tr("Can produce BlackStones.") };
            Frog = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.Frog, Icon = 510, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 11, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 11, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (11x11 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.")
            };
            Monkey = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.BabyMonkey, Icon = 511, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 11, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 11, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (11x11 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.")
            };
            AngryBird = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.AngryBird, Icon = 512, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 11, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 11, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (11x11 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.")
            };
            Foxey = new IntelligentCreatureInfo { PetType = IntelligentCreatureType.Foxey, Icon = 513, MousePickupEnabled = true, MousePickupRange = 11, AutoPickupEnabled = true, AutoPickupRange = 11, SemiAutoPickupEnabled = true, SemiAutoPickupRange = 11, CanProduceBlackStone = true,
                Info = Envir.Tr("Can pickup items (11x11 auto/semi-auto, 11x11 mouse)."), Info1 = Envir.Tr("Can produce BlackStones.")
            };
        }
