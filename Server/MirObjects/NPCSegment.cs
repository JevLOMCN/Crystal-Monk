...
namespace Server.MirObjects
{
    public class NPCSegment
    {
        ...
        {
            // G_开头
            Regex regex = new Regex(@"G_[A-Z_0-9]+");
            if (regex.Match(key).Success)
            {
                var GlobalVar = SMain.Envir.GetRegion(player.CurrentMap.Region).GlobalVar;
                for (int i = 0; i < GlobalVar.Count; i++)
                {
                    if (!string.Equals(GlobalVar[i].Key, key, StringComparison.CurrentCultureIgnoreCase)) continue;
                    GlobalVar[i] = new KeyValuePair<string, string>(GlobalVar[i].Key, value);
                    return;
                }

                SMain.EnqueueDebugging(string.Format("AddVariable {0}, {1} {2}", player.Name, key, value));
                GlobalVar.Add(new KeyValuePair<string, string>(key, value));
                return;
            }
        ...
        public void ParseCheck(string line)
        {
            ...
            switch (parts[0].ToUpper())
            {
                ...
                case "CHECKDATETIME":
                    ...
                    CheckList.Add(new NPCChecks(CheckType.CheckDateTime, parts[1], parts[2], parts[3], parts[4]));
                    break;
                ...
                case "CHECKEQUAL":
                    if (parts.Length < 3) return;
                    CheckList.Add(new NPCChecks(CheckType.CheckEqual, parts[1], parts[2]));
                    break;
                ...
                case "CHECKQUEST":
                    ...
                    break;
                case "CHECKACCEPTQUEST":
                    if (parts.Length < 2) return;
                    CheckList.Add(new NPCChecks(CheckType.CheckAcceptQuest, parts[1]));
                    break;
        ...
        public void ParseAct(List<NPCActions> acts, string line)
        {
            ...
            switch (parts[0].ToUpper())
            {
                ...
                case "CHANGEEQUIP":
                    ...
                    acts.Add(new NPCActions(ActionType.ChangeEquip, parts[1], parts[2], parts[3]));
                    break;

                case "ACCEPTQUEST":
                    if (parts.Length < 2) return;
                    acts.Add(new NPCActions(ActionType.AcceptQuest, parts[1]));
                    break;
                ...
                case "CLEARSKILL":
                    acts.Add(new NPCActions(ActionType.ClearSkill));
                    break;
                ...
                case "MOV":
                    if (parts.Length < 3) return;
                    match = Regex.Match(parts[1], @"[A-Z_0-9]+", RegexOptions.IgnoreCase);
        ...
        {
            var regex = new Regex(@"\<\$(.*)\>");                   // <>
            var varRegex = new Regex(@"(.*?)\(([A-Z][0-9])\)");     // 变量 
            var oneValRegex = new Regex(@"(.*?)\(((.*?))\)");       // 一个参数的方法 
            var twoValRegex = new Regex(@"(.*?)\(((.*?),(.*?))\)"); // 两个参数的方法
            ...
            {
                case "MONSTERCOUNT()":
                    Map map = SMain.Envir.GetMapByNameAndInstance(oneValMatch.Groups[2].Captures[0].Value.ToUpper(), player.GetRegionID());
                ...
                case "USERNAME":
                    ...
                    break;
                case "REGIONID":
                    newValue = player.GetRegionID().ToString();
                    break;
                case "LEVEL":
                ...
                case "RANKTOPLEVEL":
                    Region region = SMain.Envir.GetRegion(player.GetRegionID());
                    if (region != null && region.RankTop.Count > 0)
                        newValue = region.RankTop[0].level.ToString();
                    else
                        newValue = "0";
                    break;

                case "TIMESTAMP":
                    newValue = Functions.CurrentSecond().ToString();
                    break;

                default:
                    newValue = GetGlobal(player.CurrentMap.Region, innerMatch);
        ...
        public string ReplaceValue(MonsterObject Monster, string param)
        {
            ...
            switch (innerMatch)
            {
                ...
                case "USERNAME":
                    ...
                    break;
                case "REGION":
                    newValue = Monster.CurrentMap.Region.ToString();
                    break;
                case "LEVEL":
        ...
        private bool CheckDateTime(List<string> param)
        {
            try
            {
                DateTime begin = Convert.ToDateTime(param[0] + " " + param[1]);
                DateTime end = Convert.ToDateTime(param[2] + " " + param[3]);
        ...
        public bool Check(byte RegionID)
        {
            var failed = false;
            ...
            for (int i = 0; i < CheckList.Count; i++)
            {
                ...
                switch (check.Type)
                {
                    case CheckType.CheckDateTime:
                        failed = !CheckDateTime(param);
                        break;
                    ...
                    case CheckType.CheckHum:
                        ...
                        map = SMain.Envir.GetMapByNameAndInstance(param[2], RegionID, tempInt2);
                        if (map == null)
                        {
                            failed = true;
                            break;
                    ...
                    case CheckType.CheckExactMon:
                        ...
                        map = SMain.Envir.GetMapByNameAndInstance(param[3], RegionID, tempInt2);
                        if (map == null)
                        {
                            failed = true;
                            break;
                        ...
                        failed = (!Compare(param[1], map.Objects.Count((
                            d => d.Race == ObjectType.Monster &&
        ...
        public bool Check(MonsterObject Monster)
        {
            ...
            {
                SMain.EnqueueDebugging(string.Format("Check failed {0}", Monster.Name));
                return false;
            }
            byte regionID = Monster.CurrentMap.Region;
            ...
            for (int i = 0; i < CheckList.Count; i++)
            {
                ...
                switch (check.Type)
                {
                    ...
                    case CheckType.CheckMap:
                        map = SMain.Envir.GetMapByNameAndInstance(param[0], regionID);
        ...
        public bool Check(PlayerObject player)
        {
            ...
            for (int i = 0; i < CheckList.Count; i++)
            {
                ...
                byte regionID = player.GetRegionID();
                byte tempByte;
                uint tempUint;
                int tempInt;
                int tempInt2;
                ...
                switch (check.Type)
                {
                    ...
                    case CheckType.CheckMap:
                        Map map = SMain.Envir.GetMapByNameAndInstance(param[0], regionID);
                    ...
                    case CheckType.CheckEqual:
                        failed = string.Compare(param[0], param[1], true) != 0;
                        break;
                    ...
                    case CheckType.CheckCalc:
                        ...
                        if (!int.TryParse(param[0], out left) || !int.TryParse(param[2], out right))
                        {
                            SMain.Enqueue(string.Format("Incorrect operator: {0}, {1} Page: {1}", param[1], param[2], Key));
                            failed = true;
                            break;
                        }
                    ...
                    case CheckType.CheckQuest:
                        ...
                        break;
                    case CheckType.CheckAcceptQuest:
                        if (!int.TryParse(param[0], out tempInt))
                        {
                            failed = true;
                            break;
                        ...
                        failed = !player.CheckAcceptQuest(tempInt, true); 
                        break;
            ...
            for (var i = 0; i < acts.Count; i++)
            {
                ...
                switch (act.Type)
                {
                    case ActionType.Move:
                        ...
                        if (map.Info.NoEnter && !player.IsGM) return;
                    ...
                    case ActionType.InstanceMove:
                        ...
                        map = SMain.Envir.GetMapByNameAndInstance(param[0], regionID, instanceId);
                        if (map == null) return;
                    ...
                    case ActionType.ChangeEquip:
                        player.ChangeEquip(param);
                        break;

                    case ActionType.AcceptQuest:
                        int index;
                        if (!int.TryParse(param[0], out index)) return;
                        player.AcceptQuest(index, true);
                        break;
                    ...
                    case ActionType.Mongen:
                        ...
                        map = SMain.Envir.GetMapByNameAndInstance(Param1, regionID, Param1Instance);
                        if (map == null) return;
                    ...
                    case ActionType.MonClear:
                        ...
                        map = SMain.Envir.GetMapByNameAndInstance(param[0], regionID, tempInt);
                        if (map == null) return;

                        List<MapObject> cellObjects = map.GetAllObjects();
                        if (cellObjects == null) continue;

                            for (int j = 0; j < cellObjects.Count(); j++)
                            {
                                MapObject ob = cellObjects[j];
                    ...
                    case ActionType.GroupTeleport:
                        ...
                        for (int j = 0; j < player.GroupMembers.Count(); j++)
                        {
                            if (!SMain.Envir.CheckSameServer(player.GroupMembers[j].GetRegionID(), regionID))
                                continue;
                    ...
                    case ActionType.Calc:
                        ...
                        int.TryParse(param[0], out left);
                        int.TryParse(param[2], out right);
                    ...
                    case ActionType.ComposeMail:
                        ...
                        {
                            MailID = SMain.Envir.GenerateMailId(),
                            Sender = param[1],
                            Message = param[0]
                        };
                        break;
        ...
        private void Calc(List<string> param, MonsterObject Monster)
        {
            int left;
            int right;
        ...
        private void Act(IList<NPCActions> acts, MonsterObject Monster)
        {
            if (Monster.CurrentMap == null)
            {
                SMain.EnqueueDebugging(string.Format("Monster CurrentMap is null {0}", Monster.Name));
                return;
            }
            ...
            for (var i = 0; i < acts.Count; i++)
            {
                ...
                switch (act.Type)
                {
                    ...
                    case ActionType.Goto:
                        SMain.Envir.MonsterNPC.Call(Monster, param[0]);
                        break;
        ...
        private void Act(IList<NPCActions> acts, byte regionID)
        {
        ...
        private void Success(byte RegionID)
        {
            Act(ActList, RegionID);
        }

        private void Failed(byte RegionID)
        {
            Act(ElseActList, RegionID);
        }
        ...
        private string GetGlobal(byte region, string innerMatch)
        {
            var GlobalVar = SMain.Envir.GetRegion(region).GlobalVar;
            foreach (KeyValuePair<string, string> t in GlobalVar)
            {
                if (string.Equals(t.Key, innerMatch, StringComparison.CurrentCultureIgnoreCase)) return t.Value;
            }

            SMain.Enqueue(string.Format("Invalid GetGlobal nil {0}", innerMatch));
            return "";
        }
