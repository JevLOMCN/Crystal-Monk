...
namespace Server.MirObjects
{
    public sealed class NPCObject : MapObject
    {
        ...
        public NPCObject(NPCInfo info, Map map = null)
        {
            Info = info;
        ...
        public void LoadGoods()
        {
            if (CurrentMap == null)
            {
                SMain.Enqueue(string.Format("NPC No CurrentMap {0}", Info.Name));
                return;
            }

            string path = Settings.GoodsPath + Info.Index.ToString() + "-" + CurrentMap.Region + ".msd";
        ...
        {
            for (int j = 0; j < NPCPages.Count - 1; ++j)
            {
                if (NPCPages[NPCPages.Count - 1].Key == NPCPages[j].Key)
                {
                    SMain.EnqueueDebugging("ÖØ¸´µÄkey " + NPCPages[j].Key + ":" + j + " and " + (NPCPages.Count - 1));
                }
            }
        }
        ...
        private void ParseQuests(IList<string> lines)
        {
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                while (++i < lines.Count)
                {
                    ...
                    if (index > 0)
                        info.NpcIndex = (uint)Info.Index;
                    else
                        info.FinishNpcIndex = (uint)Info.Index;
        ...
        public void Call(byte regionID, string key) //run a verry limited npc script (should really only be used to spawn mobs or something)
        {
            key = key.ToUpper();
            ...
            for (int i = 0; i < NPCPages.Count; i++)
            {
                ...
                foreach (NPCSegment segment in page.SegmentList)
                {
                    ...
                    ProcessSegment(regionID, page, segment);
                }
            }
        }
        ...
        private void ProcessSegment(byte RegionID, NPCPage page, NPCSegment segment)
        {
            segment.Check(RegionID);
        }
        ...
        private void ProcessSpecial(PlayerObject player, NPCPage page)
        {
            ...
            switch (page.Key.ToUpper())
            {
                ...
                case MarketKey:
                    ...
                    player.GetMarket(new MarketMatcher());
                    break;
                ...
                case SocketKey:
                    ...
                    break;
                case ChangeNameKey:
                    player.Enqueue(new S.ChangeNameRequest());
                    break;
            }
        }
        ...
        public void Buy(PlayerObject player, ulong index, uint count)
        {
            ...
            uint cost = goods.Price(count);
            if (cost <= 0)
                return;

            cost = (uint)(cost * PriceRate(player));
            uint baseCost = (uint)(goods.Price(count) * PriceRate(player, true));
        ...
        public void Craft(PlayerObject player, ulong index, uint count, int[] slots)
        {
            ...
            UserItem craftedItem = Envir.CreateDropItem(goods.Info);
            ...
            foreach (var ingredient in recipe.Ingredients)
            {
                ...
                for (int i = 0; i < slots.Length; i++)
                {
                    ...
                    if (recipe.BaseItemIndex != 0 && recipe.BaseItemIndex == item.Info.Index)
                    {
                        craftedItem.InheritItem(item);
                    }

                    break;
                }

                player.UpdateQuestItem(ingredient.Info);
            }

            if (Envir.Random.Next(100) >= recipe.SuccessRate + player.CraftRate)
            {
                p.Success = false;
                player.Enqueue(p);
            }
