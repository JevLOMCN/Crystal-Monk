...
namespace Server.MirObjects
{
    public class IntelligentCreatureObject : MonsterObject
    {
        ...
        {
            get { return Master == null ? CustomName : (Dead ? CustomName : string.Format(Envir.Tr("{0}_{1}'s Pet"), CustomName, Master.Name)); }
                ...
                if (item.Item != null)
                {
                    ...
                    if (item.Item.Info.ShowGroupPickup && IsMasterGroupMember(Master))
                        for (int j = 0; j < Master.GroupMembers.Count; j++)
                            Master.GroupMembers[j].ReceiveChat(Envir.Format("{0} Picked up: {1}", Name, item.Item.Name), ChatType.Hint);

                    if (item.Item.Info.Grade == ItemGrade.Mythical || item.Item.Info.Grade == ItemGrade.Legendary)
                    {
                        Master.ReceiveChat(Envir.Format("Pet Picked up: {0}", item.Item.Name), ChatType.Hint);
