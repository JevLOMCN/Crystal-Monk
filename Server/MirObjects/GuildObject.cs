...
namespace Server.MirObjects
{
    ...
    public class GuildObject
    {
        ...
        {
            Name = name;
            Rank Owner = new Rank() { Name = Envir.Tr("Leader"), Options = (RankOptions)255 , Index = 0};
        ...
        public bool ChangeNickName(PlayerObject Self, string NickName)
        {
            for (int i = 0; i < Ranks.Count; i++)
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                {
                    ...
                    {
                        Ranks[i].Members[j].name = NickName;
                        return true;
                    }
                }
