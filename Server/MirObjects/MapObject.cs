...
namespace Server.MirObjects
{
    public abstract class MapObject
    {
        ...
        public Map CurrentMap
        {
            set
            {
                if (HasSpawn && _currentMap != null)
                {
                    _currentMap.Objects.Remove(Node);
                }
                ...
                if (HasSpawn && _currentMap != null)
                {
                    Node = _currentMap.Objects.AddLast(this);
                }
            }
        ...
        public short MagicResist, PoisonResist, HealthRecovery, SpellRecovery, PoisonRecovery, CriticalRate, CriticalDamage, Holy, Freezing, PoisonAttack;
        ...
        public long ChannelingTime;
        public int ChannelingId;
        public int SpellingId;

        public int NextChannelingId;
        ...
        public List<BuffV2> BuffV2s = new List<BuffV2>();
        ...
        private bool HasSubHp;

        public bool MagicShield;
        public byte MagicShieldLv;
        public long MagicShieldTime;

        public bool MagicShield1;
        public byte MagicShieldLv1;
        public long MagicShieldTime1;
        ...
        public virtual void Process()
        {
            ...
            if (NextChannelingId != 0 && Envir.Time > ChannelingTime)
            {
                ChannelingId = 0;
                ChannelingTime = 0;
            }
        ...
        public int GetAttackPower(int min, int max)
        {
            ...
            if (Buffs.Any(e => e.Type == BuffType.StormEscape))
                return (int)(Envir.Random.Next(min, max + 1) * 1.1F);
        ...
        public bool CanFly(Point target)
        {
            ...
            while (location != target)
            {
                ...
                if (!CurrentMap.CanWalk(location)) return false;
            }
        ...
        public virtual void Spawned()
        {
            ...
            if (CurrentMap != null)
                Node = CurrentMap.Objects.AddLast(this);
            else
                SMain.Enqueue(string.Format("Current Map is null {0} {1}", Name, GetType()));
        ...
        {
            Broadcast(new S.ObjectRemove {ObjectID = ObjectID});
            CurrentMap.Objects.Remove(Node);
        ...
        public int Attacked(MapObject attacker, int damage, DefenceType type = DefenceType.ACAgility, bool damageWeapon = true)
        {
            if (attacker is PlayerObject)
                return Attacked((PlayerObject)attacker, damage, type, damageWeapon);
            else
                return Attacked((MonsterObject)attacker, damage, type);
        }

        public abstract int Attacked(PlayerObject attacker, int damage, DefenceType type = DefenceType.ACAgility, bool damageWeapon = true, bool isReflectDamage = false);
        ...
        public virtual void AddBuffV2(BuffV2 b)
        {
            BuffV2s.Add(b);
        }

        public virtual void RemoveBuffV2(BuffType b)
        {
            for (int i = 0; i < BuffV2s.Count; i++)
            {
                if (BuffV2s[i].Type != b) continue;
                BuffV2s[i].Expire = 0;
            }
        }
        ...
        public virtual bool TeleportOut(Point location, bool effects = true, byte effectnumber = 0)
        {
            ...
            DelayedAction action = new DelayedAction(DelayedType.Teleport, Envir.Time + 2500, this, location, true, effectnumber);
           ...
           if (CurrentMap != null)
            {
                ...
                {
                    if (HasSubHp || Health != MaxHealth || Envir.Time <= RevTime)
                    {
                        HasSubHp = true;
                        CurrentMap.Broadcast(p, CurrentLocation);
                        return;
        ...
        public void MoreaBlood(int damage, int duration)
        {
            AddBuff(new Buff { Type = BuffType.MoreaBlood, Caster = this, ExpireTime = Envir.Time + duration, Values = new int[] { damage } });
        }

        public void MoreaBlood2(int damage, int duration)
        {
            AddBuff(new Buff { Type = BuffType.MoreaBlood2, Caster = this, ExpireTime = Envir.Time + duration, Values = new int[] { damage } });
        }

        public void MoreaBlood3(int damage, int duration)
        {
            AddBuff(new Buff { Type = BuffType.MoreaBlood3, Caster = this, ExpireTime = Envir.Time + duration, Values = new int[] { damage } });
        }

        public void MoreaBlood8(int damage, int duration)
        {
            AddBuff(new Buff { Type = BuffType.MoreaBlood8, Caster = this, ExpireTime = Envir.Time + duration, Values = new int[] { damage } });
        }
        ...
        public void TeleportIn(Point location, bool effects, byte effectnumber)
        {
            CurrentLocation = location;
        ...
        public bool InTrapRock
        {
            ...
            get
            {
                ...
                for (int i = 0; i <= 6; i += 2)
                {
                    ...
                    var cellObjects = CurrentMap.GetCellObjects(checklocation.X, checklocation.Y);
                    if (cellObjects == null) continue;
