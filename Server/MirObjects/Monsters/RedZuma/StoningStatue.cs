...
namespace Server.MirObjects.Monsters
{
    public class StoningStatue : MonsterObject
    {
        ...
        {
            List<MapObject> targets = FindAllTargets(8, Functions.PointMove(CurrentLocation, Direction, 2), false);
            ...
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                if (Envir.Random.Next(2) == 0)
                {
                    ...
                    {
                        targets[i].ApplyPoison(new Poison { Owner = this, PType = PoisonType.Stun, Duration = 5, TickSpeed = 1000 }, this);
                        Broadcast(new S.ObjectEffect { ObjectID = targets[i].ObjectID, Effect = SpellEffect.Stunned, Time = (uint)5000 });
                    }
                }

                Broadcast(new S.ObjectEffect { ObjectID = targets[i].ObjectID, Effect = SpellEffect.StoningStatusFire, Direction = Direction});
            }
        }
