                    ...
                    for (int k = 0; k < cellObjects.Count; k++)
                    {
                        ...
                        switch (target.Race)
                        {
                            ...
                            case ObjectType.Player:
                                if (target.IsAttackTarget(this))
                                {
                                    if (target.Attacked(this, j <= 1 ? nearDamage : farDamage, j <= 1 ? DefenceType.AC : DefenceType.MAC) > 0)
                                    {
                                        if (Envir.Random.Next(Settings.PoisonResistWeight) >= target.PoisonResist)
                                        {
                                            ...
                                            if (Envir.Random.Next(5) == 0)
                                            {
                                                ...
                                                {
                                                    Owner = this,
                                                    Duration = target.Race == ObjectType.Player ? 4 : 5 + Envir.Random.Next(this.Freezing),
