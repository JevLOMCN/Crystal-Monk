...
namespace Server.MirObjects.Monsters
{
    public class Trainer : MonsterObject
    {
        ...
        public override int Attacked(PlayerObject attacker, int damage, DefenceType type = DefenceType.ACAgility, bool damageWeapon = false, bool isReflectDamage = false)
        {
        ...
        public override void PoisonDamage(int damage, MapObject attacker)
        {
            ...
            _currentAttacker.ReceiveChat(string.Format("{1}造成了 {0} 伤害, 每秒伤害: {2:#.00}.", damage, attacker is MonsterObject ? "你宠物的毒" : "你的毒", Dps), ChatType.Trainer);
            Poisoned = true;
        }
        ...
        public override void ChangeHP(int amount)
        {
            ...
            if (_StartTime == 0)
                timespend = 1000;
            double Dps = _totalDamage / (timespend * 0.001);
            _currentAttacker.ReceiveChat(string.Format("你的毒造成了 {0} 伤害, 每秒伤害: {1:#.00}.", amount, Dps), ChatType.Trainer);
        }
        ...
        private void ReportDamage(int damage, DefenceType type, bool Pet)
        {
            ...
            if (_StartTime == 0)
                timespend = 1000;
            double Dps = _totalDamage / (timespend * 0.001);
            _currentAttacker.ReceiveChat(string.Format("{3}造成了 {0} {1} 伤害, 每秒伤害: {2:#.00}.", damage, output, Dps, Pet? "你的宠物": "你"), ChatType.Trainer);
        }
       ...
       //     PoisonList.Clear();
        ...
        private void OutputAverage()
        {
            ...
            if (_StartTime == 0)
                timespend = 1000;
            double Dps = _totalDamage / (timespend * 0.001);
            _currentAttacker.ReceiveChat(string.Format("{0} 每次平均伤害, 秒伤: {1:#.00}.", (int)(_totalDamage / _hitCount),Dps), ChatType.Trainer);
        }
    }
}
