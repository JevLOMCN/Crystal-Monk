...
namespace Server.MirEnvir
{
    public class Reporting
    {
        ...
        public void ItemChanged(string source, UserItem item, uint amount, int state)
        {
            ...
            if (item != null)
            {
                task = string.Format("Item {0} - {1} x{2} left:{3} ({4})", type, item.Info.Name, amount, item.Count, item.UniqueID);
            }
        ...
        public void GoldChanged(string source, uint amount, uint gold, bool lost = true, string info = "")
        {
            string task = string.Format("Gold{0} - x{1} left {2}", lost ? "Lost" : "Gained", amount, gold);
        ...
        public void CreditChanged(string source, uint amount, uint left, bool lost = true, string info = "")
        {
            string task = string.Format("Credit{0} - x{1} left {2}", lost ? "Lost" : "Gained", amount, left);
        ...
        private void Save()
        {
            ...
            string baseDir = Settings.ReportPath + SMain.Envir.Now.Date.ToString(@"yyyy-MM-dd") + @"\" + Player.Account.AccountID;
            if (!Directory.Exists(baseDir))
                Directory.CreateDirectory(baseDir);

            string fullPath = baseDir + @"\" + Player.Name;
