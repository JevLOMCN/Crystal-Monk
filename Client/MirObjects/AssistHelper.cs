...
namespace Client.MirObjects
{
    public class AutoSkill
    {
        ...
        public virtual bool CheckState()
        {
            switch(Spell)
            {
                ...
                case Spell.FlamingSword:
                    return GameScene.NextTimeFireHit || CMain.Time - GameScene.LastFireHitTick < 10000;

                case Spell.DaMoGunFa:
                    return GameScene.NextTimeDaMoHit || CMain.Time - GameScene.LastDamoHitTick < 10000;

                case Spell.ElementalBarrier:
                    return User.ElementalBarrier;

                case Spell.ElementalBarrier1:
                    return User.ElementalBarrier1;
            }
    ...
    public class AssistHelper
    {
        ...
        public AssistHelper()
        {
            ...
            AutoList.Add(new AutoSkill(Spell.ElementalBarrier1, 1000));
        }
        ...
        private bool ProcessSpell()
        {
                ...
                && TargetObject != null && !TargetObject.Poison.HasFlag(PoisonType.Green) && CMain.Time > MapControl.NextAction)
            {
                ...
                NextActionTime += 2500;
                GameScene.Scene.MapControl.UseMagic(magic);
                return true;
            }

            if (User.GetMagic(Spell.Haste) != null && !User.IsMagicInCD(Spell.Haste) && !GameScene.Scene.Buffs.Any(e => e.Type == BuffType.Haste))
            {
            ...
            if (User.GetMagic(Spell.LightBody) != null && !User.IsMagicInCD(Spell.LightBody) && !GameScene.Scene.Buffs.Any(e => e.Type == BuffType.LightBody))
            {
        ...
        private bool CheckEquipment(int slot, ItemType itemType, int count, int shape)
        {
            ...
            return item != null && item.Info.Type == ItemType.Amulet && item.Count >= count && item.Info.Shape == shape;
        }
        ...
        private bool AutoEquipAmulet(byte shape, byte count)
        {
            ...
            if (shape != 0)
            {
                if (CheckEquipment((int)EquipmentSlot.AmuletUp, ItemType.Amulet, count, shape))
                    return true;
                ...
                Network.Enqueue(new C.EquipItem { Grid = MirGridType.Inventory, UniqueID = item.UniqueID, To = (int)EquipmentSlot.AmuletUp });
            }
            else
            {
                if (CheckEquipment((int)EquipmentSlot.Amulet, ItemType.Amulet, count, shape))
                    return true;
