...
namespace Client.MirObjects
{
    public class PlayerObject : MapObject
    {
        ...
        public bool HasClassWeapon
        {
            get
            {
                return Functions.HasClassWeapon(Class, Weapon);
            }
        }
        ...
        public Effect ElementalBarrierEffect1;

        public bool[] Status = new bool[10];
        public Effect[] StatusEffect = new Effect[10];
      ...
      //  public bool HasElements;
        ...
        public long ChannelingTime, ReincarnationStopTime;

        public short MountType = -1, TransformType = -1;
        public Effect TransformEffect;
        ...
        public void UpdateAnimCtrl()
        {
            switch (Class)
            {
                case MirClass.Warrior:
                    AnimCtrl = new WarriorAnimCtrl();
                    break;

                case MirClass.Wizard:
                    AnimCtrl = new WizardAnimCtrl();
                    break;

                case MirClass.Taoist:
                    AnimCtrl = new TaoistAnimCtrl();
                    break;

                case MirClass.Assassin:
                    AnimCtrl = new AssassinAnimCtrl();
                    break;

                case MirClass.Archer:
                    AnimCtrl = new ArcherAnimCtrl();
                    break;

                case MirClass.Monk:
                    AnimCtrl = new MonkAnimCtrl();
                    break;
            }
        }
        ...
        public virtual void SetLibraries()
        {
            ...
            if (TransformType > -1 && Settings.ShowTransform)
            {
                ...
                if (CurrentAction != MirAction.JumpDown)
                {
            ...
            {
                AnimCtrl.Weapon = Weapon;
                AnimCtrl.Armour = Armour;
                AnimCtrl.Hair = Hair;
                AnimCtrl.WeaponEffect = WeaponEffect;
                AnimCtrl.WingEffect = WingEffect;
                AnimCtrl.HasClassWeapon = HasClassWeapon;
                AnimCtrl.HumUp = LookHumUp;
                AnimCtrl.Gender = Gender;
                AnimCtrl.MountType = MountType;
                AnimCtrl.RidingMount = RidingMount;
                AnimCtrl.ShowMount = showMount;

                AnimCtrl.SetLibraries(CurrentAction);

                BodyLibrary = AnimCtrl.BodyLibrary;
                WeaponLibrary1 = AnimCtrl.WeaponLibrary1;
                WeaponEffectLibrary1 = AnimCtrl.WeaponEffectLibrary1;
                WeaponLibrary2 = AnimCtrl.WeaponLibrary2;
                HairLibrary = AnimCtrl.HairLibrary;
                WingLibrary = AnimCtrl.WingLibrary;
                MountLibrary = AnimCtrl.MountLibrary;

                ArmourOffSet = AnimCtrl.ArmourOffSet;
                HairOffSet = AnimCtrl.HairOffSet;
                WeaponOffSet = AnimCtrl.WeaponOffSet;
                WingOffset = AnimCtrl.WingOffset;
                MountOffset = AnimCtrl.MountOffset;
                ...
                if (LookHumUp)
                {
                    ...
                    {
                        case MirClass.Warrior:
                            ResetFramePos(AnimCtrl.AltAnim, 1);
                            break;

                        case MirClass.Wizard:
                            ResetFramePos(AnimCtrl.AltAnim, 2);
                            break;

                        case MirClass.Taoist:
                            ResetFramePos(AnimCtrl.AltAnim, 3);
                            break;

                        case MirClass.Assassin:
                            ResetFramePos(AnimCtrl.AltAnim, 4);
                            break;

                        case MirClass.Archer:
                            ResetFramePos(AnimCtrl.AltAnim, 5);
                            break;
                    }
                }
                else
        ...
        {
          //  CMain.SaveDebug(string.Format("=>开始动作切换 {0} {1}", CMain.Time, CurrentAction));
            ...
            {
        //        CMain.SaveDebug(string.Format("切换失败 时间未达到 {0} {1}", CMain.Time, MapControl.NextAction));
                //NextMagic = null;
                return;
            }
            ...
            if (ActionFeed.Count == 0)
            {
               ...
               CurrentAction = CMain.Time > ChannelingTime ? CurrentAction : MirAction.Stance2;
            ...
            if (this == User)
            {
         //       CMain.SaveDebug(string.Format("=>动作切换完毕 {0} {1} 上一个动作执行耗时: {2}", CMain.Time, CurrentAction, CMain.Time - User.LastSetActionTime));
                User.LastSetActionTime = CMain.Time;
            }
            ...
            if (ElementalBarrier1)
            {
                switch (CurrentAction)
                {
                    case MirAction.Struck:
                    case MirAction.MountStruck:
                        if (ElementalBarrierEffect1 != null)
                        {
                            ElementalBarrierEffect1.Clear();
                            ElementalBarrierEffect1.Remove();
                        }

                        Effects.Add(ElementalBarrierEffect1 = new Effect(Libraries.Magic3, 1910, 5, 600, this));
                        ElementalBarrierEffect1.Complete += (o, e) => Effects.Add(ElementalBarrierEffect1 = new Effect(Libraries.Magic3, 1890, 16, 3200, this) { Repeat = true });
                        break;
                    default:
                        if (ElementalBarrierEffect1 == null)
                            Effects.Add(ElementalBarrierEffect1 = new Effect(Libraries.Magic3, 1890, 16, 3200, this) { Repeat = true });
                        break;
                }
            }

            if ((TransformType == 27 && Settings.ShowTransform))
            {
                if (TransformEffect == null)
                    Effects.Add(TransformEffect = new Effect(Libraries.Magic3, 3470, 8, 1000, this, 0, false) { Repeat = true});
            }
            else if ((Armour % 100 == 5 && LookHumUp) || Armour == 30)
            {
                if (TransformEffect == null)
                    Effects.Add(TransformEffect = new Effect(Libraries.Magic3, 3450, 10, 1500, this, 0, true) { Repeat = true });
            }
            else
            {
                if (TransformEffect != null)
        ...
        private void SetFrames(QueuedAction action, MirDirection oldDir)
        {
            ...
            switch (CurrentAction)
            {
                ...
                case MirAction.MountRunning:
                    Frames.Frames.TryGetValue(MirAction.MountRunning, out Frame);
                    CancelChannelling();
                    if (User == this)
                    {
                        GameScene.LastRunTime = CMain.Time;
                ...
                case MirAction.Pushed:
                    if (this == User)
                        GameScene.Scene.MapControl.InputDelay = CMain.Time + 500;
                    Frames.Frames.TryGetValue(MirAction.Walking, out Frame);
                ...
                case MirAction.Attack1:
                    switch (Class)
                    {
                        ...
                        case MirClass.Assassin:
                            if (User == this)
                            {
                                if (User.IsMagicToggle(Spell.DoubleSlash))
                                    Frames.Frames.TryGetValue(MirAction.Attack1, out Frame);
                                else if (CMain.Shift)
                            ...
                            break;
                        default:
                            if (User == this && CMain.Shift && TargetObject == null)
                ...
                case MirAction.Spell:
                    UpdateSpellFrames(action, oldDir);
                    break;
                ...
                case MirAction.AttackRange2:
                    ...
                    if (User == this)
                    {
                        ...
                        if (Spell != Spell.PoisonShot)
                        {
                            ...
                            if (Spell == Spell.FlashDash || Spell == Spell.FlashDash2)
                            {
                                GameScene.SpellTime = CMain.Time + User.AttackSpeed;
                ...
                case MirAction.AttackRange1:
                    ...
                    if (User == this)
                    {
                        ...
                        ClientMagic magic = User.GetMagic(Spell.PoisonShot);
                        if (magic != null && magic.Toggle && User.CheckMagicMP(magic) && MapObject.TargetObject != null)
                        {
                ...
                case MirAction.Attack1:
                    ...
                    switch (Spell)
                    {
                        ...
                        case Spell.DoubleSlash:
                            ...
                            {
                                var user = (UserObject)this;
                                FrameInterval = User.AttackSpeed /  (2 * Frame.Count);
                                EffectFrameInterval = User.AttackSpeed / (2 * Frame.EffectCount);
                                //FrameInterval = (int)((FrameInterval - 2 * User.ASpeed) * 0.5); 
                                //EffectFrameInterval = (int)(0.5 * (EffectFrameInterval - 2 * User.ASpeed)); 
                            }
                            else
                            {
                            ...
                            ActionFeed.Insert(0, action);
                            if (this == MapControl.User)
                                MapControl.NextAction = CMain.Time + FrameInterval * Frame.Count;// 2500;
                    ...
                    switch (Spell)
                    {
                        case Spell.DoubleSlash:
                            if (this == MapObject.User)
                            {
                                ...
                                FrameInterval = User.AttackSpeed / (2 * Frame.Count);
                                EffectFrameInterval = User.AttackSpeed / (2 * Frame.EffectCount);
                              //  FrameInterval = (int)(0.5 * FrameInterval - 2 * User.ASpeed);
                              //  EffectFrameInterval = (int)(0.5 * EffectFrameInterval - 2 * User.ASpeed);
                            }
                            else
                            {
        ...
        private void UpdateSpellFrames(QueuedAction action, MirDirection oldDir)
        {
            Spell = (Spell)action.Params[0];
            uint targetID = (uint)action.Params[1];
            Point location = (Point)action.Params[2];
            switch (Spell)
            {
                ...
                case Spell.JinGangGunFa:
                  ...
                  case Spell.CounterAttack1:
                    // TODO?
                    // Frames.Frames.TryGetValue(MirAction.Spell, out Frame);
                    if (this == User)
                    {
                ...
                case Spell.FlashDash2:
                    {
                        int level = (byte)action.Params[3];
                        GetFlashDashDistance(level);
                        if (JumpDistance != 0)
                        {
                        ...
                        {
                            MapControl.NextAction = CMain.Time;
                            GameScene.SpellTime = CMain.Time + User.AttackSpeed; //Spell Delay
                            if (JumpDistance != 0)
                ...
                case Spell.DoubleShot:
                    ...
                    if (this == User)
                    {
                        ...
                        GameScene.SpellTime = CMain.Time + 1800; //Spell Delay
                    }
                    break;
                case Spell.ExplosiveTrap:
                    if (this == User)
                    {
                        if (TrapTime <= 0 || CMain.Time > TrapTime)
                        {
                            CurrentAction = MirAction.Harvest;
                            TrapTime = CMain.Time + 15000;
                            MapControl.NextAction = CMain.Time + 1000;
                            GameScene.SpellTime = CMain.Time + 500; //Spell Delay
                        }
                        else
                    ...
                    {
                        bool param = (bool)action.Params[5];
                        CurrentAction = param ?  MirAction.Spell : MirAction.Harvest;
                    }
                    ArcherLayTrap = CurrentAction == MirAction.Harvest; 
                    Frames.Frames.TryGetValue(CurrentAction, out Frame);
                    break;
               ...
               case Spell.PoisonShot:
                    ...
                    if (this == User)
                    {
                        MapControl.NextAction = CMain.Time + 550;
                        GameScene.AttackTime = CMain.Time + User.AttackSpeed;
                    }
                    break;
                ...
                default:
                    ...
                    if (this == User)
                    {
                        ...
                        else if (Spell == Spell.GreateFireBallSecret)
                        {
                            FrameIntervals[4] = 1000;
            ...
            if (this == User && Spell != Spell.FlashDash && Spell != Spell.FlashDash2)
                Network.Enqueue(new C.Magic { Spell = Spell, Direction = Direction, TargetID = targetID, Location = location });
        }
        ...
        private void CompleteCast()
        {
            ...
            switch (Spell)
            {
                ...
                case Spell.CatTongue:
                    ...
                    {
                        if (missile.Target.CurrentAction == MirAction.Dead) return;
                        missile.Target.Effects.Add(new Effect(Libraries.Magic3, 420, 8, 1200, missile.Target));
                        SoundManager.PlaySound(20000 + (ushort)Spell.CatTongue * 10);
                    };
                    break;

                case Spell.GreateFireBallSecret:
                    missile = CreateProjectile(4430, Libraries.Magic3, true, 6, 50, 4);
                    missile.Explode = true;

                    missile.Complete += (o, e) =>
                    {
                        if (missile.Target.CurrentAction == MirAction.Dead) return;
                        missile.Target.Effects.Add(new Effect(Libraries.Magic3, 4590, 10, 1500, missile.Target));
                        SoundManager.PlaySound(20000 + (ushort)Spell.GreateFireBallSecret * 10);
                    };
                    break;
            }
        }
        ...
        public virtual void ProcessFrames()
        {
            ...
            switch (CurrentAction)
            {
                ...
                case MirAction.DashAttack:
                    ...
                    if (SkipFrames) UpdateTime();

                    if (UpdateTime(false) >= Frame.Count)
                    {
                        FrameIndex = Frame.Count - 1;
                        SetAction();
                    }
                ...
                case MirAction.Mine:
                    ...
                    //        CMain.SaveDebug(string.Format("当前动作执行完毕 {0} 帧间隔{1} 帧数{2}", CurrentAction, FrameInterval, Frame.Count));
                    ...
                    if (CMain.Time >= NextMotion)
                    {
                        ...
                        else
                        {
                            if (FrameIntervals.ContainsKey(FrameIndex))
                            {
                                ...
                                FrameIntervals.Remove(FrameIndex);
                            }
                            else
                                NextMotion += FrameInterval;
                                ...
                                case Spell.CrippleShot:
                                    ...
                                    switch (FrameIndex)
                                    {
                                        case 7:
                                            ...
                                            missile = CreateProjectile(1930 + exFrameStart, Libraries.Magic3, true, 5, 20, 5);
                                            StanceTime = CMain.Time + StanceDelay;
                                            if (missile.Target != null)
                                            {
                                                missile.Complete += (o, e) =>
                ...
                case MirAction.Spell:
                    if (CMain.Time >= NextMotion)
                    {
                        ...
                        if (SkipFrames && !IsChanneling()) UpdateTime();
        ...
        {
            return CMain.Time < ChannelingTime;
        }

        public int UpdateTime(bool skip = true)
        {
        ...
        public override void Draw()
        {
            ...
            if ((Hidden || Hidden2) && !DXManager.Blending) DXManager.SetOpacity(0.5F);
        ...
        public override void DrawEffects(bool effectsEnabled)
        {
            ...
            CurrentMagic = MagicMgr.Get(Spell);
            ...
            switch (CurrentAction)
            {
                case MirAction.Attack1:
                    ...
                    {
                        case Spell.Slaying:
                            Libraries.Magic.DrawBlendEx(1820 + ((int)Direction * 10) + SpellLevel * 90 + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.DoubleSlash:
                            Libraries.Magic2.DrawBlendEx(1960 + ((int)Direction * 10) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.Thrusting:
                            Libraries.Magic.DrawBlendEx(2190 + ((int)Direction * 10) + SpellLevel * 90 + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.HalfMoon:
                            Libraries.Magic.DrawBlendEx(2560 + ((int)Direction * 10) + SpellLevel * 90 + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.TwinDrakeBlade:
                            Libraries.Magic2.DrawBlendEx(220 + ((int)Direction * 20) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.CrossHalfMoon:
                            Libraries.Magic2.DrawBlendEx(40 + ((int)Direction * 10) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.FlamingSword:
                            Libraries.Magic.DrawBlendEx(3480 + ((int)Direction * 10) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                    }
                    break;
                ...
                case MirAction.Attack4:
                    ...
                    {
                        case Spell.DoubleSlash:
                            Libraries.Magic2.DrawBlendEx(2050 + ((int)Direction * 10) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.TwinDrakeBlade:
                            Libraries.Magic2.DrawBlendEx(226 + ((int)Direction * 20) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
                        case Spell.FlamingSword:
                            Libraries.Magic.DrawBlendEx(3480 + ((int)Direction * 10) + FrameIndex, DrawLocation, Color.White, true, 0.7F);
                            break;
        ...
        public void DrawCurrentEffects()
        {
            ...
            if (CurrentEffect == SpellEffect.ElementalBarrierUp1 && !ElementalBarrier1)
            {
        ...
        public void DrawMount()
        {
            ...
            bool Transform = TransformType > -1 && Settings.ShowTransform;
            int offSet = (!Transform && LookHumUp) ? 792 : 416;
            int NewDrawFrame = DrawFrame;

            if (!Transform && LookHumUp)
            {
        ...
        public override void DrawName()
        {
            ...
            if (Settings.ShowGuildName && GuildName != "")
            {
                GuildLabel.Text = GuildName;
                GuildLabel.Location = new Point(DisplayRectangle.X + (50 - GuildLabel.Size.Width) / 2, DisplayRectangle.Y - (44 - GuildLabel.Size.Height / 2) + (Dead ? 35 : 8)); //was 48 -
                GuildLabel.Draw();
            }

            NameLabel.Text = Name;
            NameLabel.Location = new Point(DisplayRectangle.X + (50 - NameLabel.Size.Width) / 2, DisplayRectangle.Y - (30 - NameLabel.Size.Height / 2) + (Dead ? 35 : 8)); //was 48 -
            NameLabel.Draw();
        }
        ...
        {
            MagicConf magicConf = CMain.MagicConf.GetConf(Spell);
            
            if (magicConf != null)
            {
                if (magicConf.SpellEffect > 0)
                {
                    EffectConf effectConf = CMain.MagicConf.GetEffectConf(magicConf.SpellEffect);
                    if (effectConf != null)
                    {
                        int frame = effectConf.DurationParam1 + effectConf.DurationParam2 * Frame.Count * FrameInterval + effectConf.DurationParam3 * FrameInterval;
                        if (effectConf.owner)
                            Effects.Add(new Effect(effectConf.Lib, effectConf.BaseIndex + (int)Direction * effectConf.DirectionOffset, effectConf.Count, frame, this,
                                CMain.Time + effectConf.StartTime) { FrameIntervals = effectConf.FrameIntervals});
                        else
                            MapControl.Effects.Add(new Effect(effectConf.Lib, effectConf.BaseIndex + (int)Direction * effectConf.DirectionOffset, effectConf.Count, frame, CurrentLocation, CMain.Time + effectConf.StartTime ) { FrameIntervals = effectConf.FrameIntervals });
                    }

                    if (magicConf.SpellEffect2 > 0)
                    {
                        effectConf = CMain.MagicConf.GetEffectConf(magicConf.SpellEffect2);
                        if (effectConf != null)
                        {
                            int frame = effectConf.DurationParam1 + effectConf.DurationParam2 * Frame.Count * FrameInterval + effectConf.DurationParam3 * FrameInterval;
                            if (effectConf.owner)                              
                                Effects.Add(new Effect(effectConf.Lib,
                                    effectConf.BaseIndex + (int)Direction * effectConf.DirectionOffset,
                                    effectConf.Count,
                                    frame,
                                    this,
                                    CMain.Time + effectConf.StartTime
                                    )
                                { Repeat = effectConf.Repeat, RepeatUntil = CMain.Time + effectConf.RepeatUntil});
                            else
                                MapControl.Effects.Add(new Effect(effectConf.Lib, 
                                    effectConf.BaseIndex + (int)Direction * effectConf.DirectionOffset, 
                                    effectConf.Count, 
                                    frame, 
                                    CurrentLocation
            ...
            switch (Spell)
            {
                ...
                case Spell.Blizzard:
                    if (!Buffs.Any(x => x == BuffType.Bisul))
                        ChannelingTime = CMain.Time + Globals.BlizzardCastTime + Globals.BlizzardGuideTime - 1500;
                    break;
                ...
                case Spell.MeteorStrike:
                    if (!Buffs.Any(x => x == BuffType.Bisul))
                        ChannelingTime = CMain.Time + Globals.BlizzardCastTime + Globals.BlizzardGuideTime;
                    break;
        ...
        public void CancelChannelling()
        {
            if (ChannelingTime > 0)
            {
                if (this == UserObject.User)
                    GameScene.Scene.OutputMessage(CMain.Tr("Spell was interrupted."));
