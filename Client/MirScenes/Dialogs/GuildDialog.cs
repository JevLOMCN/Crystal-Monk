...
namespace Client.MirScenes.Dialogs
{
    ...
    public sealed class GuildDialog : MirImageControl
    {
        ...
        public GuildDialog()
        {
            ...
            for (int i = 0; i < MembersName.Length; i++)
            {
                MembersName[i] = new MirLabel()
                {
                    ...
                    Parent = MembersPage,
                    Size = new Size(100, 14),
                    Location = new Point(125, 30 + (i * 17)),
                    Visible = false,
                    Enabled = false,
                    Font = new Font(Settings.FontName, 8F)
            ...
            for (int i = 0; i < MembersStatus.Length; i++)
            {
                MembersStatus[i] = new MirLabel()
                {
                    ...
                    Parent = MembersPage,
                    Size = new Size(100, 14),
                    Location = new Point(225, 30 + (i * 17)),
                    Visible = false,
                    Enabled = false,
                    Font = new Font(Settings.FontName, 8F)
                };
            ...
            for (int i = 0; i < MembersDelete.Length; i++)
            {
                ...
                MembersDelete[i] = new MirButton()
                {
                    Enabled = true,
                    Visible = true,
                    Location = new Point(210, 30 + (i * 17)),
                    Library = Libraries.Prguse,
                    Index = 917,
                    Parent = MembersPage
                };
            ...
            String[] Options = { CMain.Tr("Edit ranks"),
                CMain.Tr("Recruit member"),
                CMain.Tr("Kick member"),
                CMain.Tr("Store item"),
                CMain.Tr("Retrieve item"),
                CMain.Tr("Alter alliance"),
                CMain.Tr("Change notice"),
                CMain.Tr("Activate Buff") };
        ...
        public string ShowStats(GuildBuffInfo buff)
        {
            ...
            if (buff.BuffAc > 0)
            {
                text += CMain.Format("Increases AC by: 0-{0}.", buff.BuffAc);
                if (text != "") text += "\n";
            }
            if (buff.BuffMac > 0)
            {
                text += CMain.Format("Increases MAC by: 0-{0}.", buff.BuffMac);
                if (text != "") text += "\n";
            }
            if (buff.BuffDc > 0)
            {
                text += CMain.Format("Increases DC by: 0-{0}.", buff.BuffDc);
                if (text != "") text += "\n";
            }
            if (buff.BuffMc > 0)
            {
                text += CMain.Format("Increases MC by: 0-{0}.", buff.BuffMc);
                if (text != "") text += "\n";
            }
            if (buff.BuffSc > 0)
            {
                text += CMain.Format("Increases SC by: 0-{0}.", buff.BuffSc);
                if (text != "") text += "\n";
            }
            if (buff.BuffMaxHp > 0)
            {
                text += CMain.Format("Increases Hp by: {0}.", buff.BuffMaxHp);
                if (text != "") text += "\n";
            }
            if (buff.BuffMaxMp > 0)
            {
                text += CMain.Format("Increases MP by: {0}.", buff.BuffMaxMp);
                if (text != "") text += "\n";
            }
            if (buff.BuffHpRegen > 0)
            {
                text += CMain.Format("Increases Health regen by: {0}.", buff.BuffHpRegen);
                if (text != "") text += "\n";
            }
            if (buff.BuffMPRegen > 0)
            {
                text += CMain.Format("Increases Mana regen by: {0}.", buff.BuffMPRegen);
                if (text != "") text += "\n";
            }
            if (buff.BuffMineRate > 0)
            {
                text += CMain.Format("Increases Mining success by: {0}%.", buff.BuffMineRate * 5);
                if (text != "") text += "\n";
            }
            if (buff.BuffGemRate > 0)
            {
                text += CMain.Format("Increases Gem success by: {0}%.", buff.BuffGemRate * 5);
                if (text != "") text += "\n";
            }
            if (buff.BuffFishRate > 0)
            {
                text += CMain.Format("Increases Fishing success by: {0}%.", buff.BuffFishRate * 5);
                if (text != "") text += "\n";
            }
            if (buff.BuffExpRate > 0)
            {
                text += CMain.Format("Increases Experience by: {0}%.", buff.BuffExpRate);
                if (text != "") text += "\n";
            }
            if (buff.BuffCraftRate > 0)
            {
                text += CMain.Format("Increases Crafting success by: {0}%.", buff.BuffCraftRate * 5);
                if (text != "") text += "\n";
            }
            if (buff.BuffSkillRate > 0)
            {
                text += CMain.Format("Increases Skill training by: {0}.", buff.BuffSkillRate);
                if (text != "") text += "\n";
            }
            if (buff.BuffAttack > 0)
            {
                text += CMain.Format("Increases Damage by: {0}.", buff.BuffAttack);
                if (text != "") text += "\n";
            }
            if (buff.BuffDropRate > 0)
            {
                text += CMain.Format("Droprate increased by: {0}%.", buff.BuffDropRate);
                if (text != "") text += "\n";
            }
            if (buff.BuffGoldRate > 0)
            {
                text += CMain.Format("Goldrate increased by: 0-{0}.", buff.BuffGoldRate);
                if (text != "") text += "\n";
            }
