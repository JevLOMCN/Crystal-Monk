...
namespace Client.MirScenes.Dialogs
{
    public sealed class IntelligentCreatureDialog : MirImageControl
    {
        ...
        public IntelligentCreatureDialog()
        {
            ...
            CreatureRenameButton.Click += RenameButtonClick;
            ...
            CreatureMaintainFoodBuff = new MirLabel
            {
                ...
                Size = new Size(166, 21),
                NotControl = true,
                Visible = true //FAR made invisible as position was wierd - not sure where it's meant to be displayed
            };
        ...
        private void BlackStoneImageFG_AfterDraw(object sender, EventArgs e)
        {
            ...
            double remainTime = GetBlackStoneTime(selectedCreature);
            double percent = 1F - remainTime / blackstoneProduceTime;
            if (percent > 1) percent = 1;
            if (percent <= 0) return;
            ...
            if (HoverLabel.Visible && HoverLabelParent != null && HoverLabelParent == BlackStoneImageFG)
            {
                ...
                {
                    HoverLabel.Text = string.Format("{0}", Functions.PrintTimeSpanFromSeconds(GetBlackStoneTime(selectedCreature)));
                }
                else
                    HoverLabel.Text = CMain.Tr("No Production.");
            }
        ...
        private double GetBlackStoneTime(int selectedCreature)
        {
            UserObject User = GameScene.User;
            long remainTime = 0;
            if (User.IntelligentCreatures[selectedCreature].PetType == GameScene.User.SummonedCreatureType)
            {
                remainTime = GameScene.User.IntelligentCreatures[selectedCreature].BlackstoneTime + CMain.Time / 1000;
                remainTime = blackstoneProduceTime - Math.Max(0, remainTime);
            }
            else
            {
                remainTime = blackstoneProduceTime - User.IntelligentCreatures[selectedCreature].BlackstoneTime;
            }
        ...
        private void RenameButtonClick(object sender, EventArgs e)
        {
            ...
            MirInputBox inputBox = new MirInputBox(CMain.Tr("Please enter a new name for the creature."));
        ...
        private void ButtonClick(object sender, EventArgs e)
        {
            ...
            {
                MirInputBox verificationBox = new MirInputBox(CMain.Tr("Please enter the creature's name for verification."));
                verificationBox.OKButton.Click += (o1, e1) =>
                {
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(CMain.Tr("Verification Failed!!"), ChatType.System);
                    }
                    else
                    {
            ...
            if (sender == SemiAutoModeButton)
            {
                ...
                {
                    GameScene.Scene.ChatDialog.ReceiveChatTr("The pet not allow auto pick up.", ChatType.System);
                    return;
                }
            ...
            if (sender == AutomaticModeButton)
            {
                ...
                {
                    GameScene.Scene.ChatDialog.ReceiveChatTr("The pet not allow Semi pick up.", ChatType.System);
                    return;
                }
        ...
        private void DrawCreatureAnimation()
        {
            ...
            //Expire
            long expireTime = GetExpireRemainTime(selectedCreature);
            if (GameScene.User.IntelligentCreatures[selectedCreature].ExpireTime == -9999)
                CreatureDeadline.Text = CMain.Tr("Expire: Never");
            else
            {
                CreatureDeadline.Text = string.Format(CMain.Tr("Expire: {0}"), Functions.PrintTimeSpanFromSeconds(expireTime));
            }

            //
            long remainTime = GetMainTainFoodTime(selectedCreature);
            CreatureMaintainFoodBuff.Text = string.Format(CMain.Tr("FoodBuff: {0}"), Functions.PrintTimeSpanFromSeconds(remainTime));
        ...
        public void Show()
        {
            ...
            {
                MirMessageBox messageBox = new MirMessageBox(CMain.Tr("You do not own any creatures."), MirMessageBoxButtons.OK);
                messageBox.Show();
                return;
            }
        ...
        private long GetMainTainFoodTime(int selectedCreature)
        {
            ...
            else
            {
                if (GameScene.User.IntelligentCreatures[selectedCreature].PetType == GameScene.User.SummonedCreatureType)
                {
                    long remainTime = GameScene.User.IntelligentCreatures[selectedCreature].MaintainFoodTime - CMain.Time / 1000;
                    return Math.Max(0, remainTime);
                }
                else
                {
                    return GameScene.User.IntelligentCreatures[selectedCreature].MaintainFoodTime;
                }
            }
        }

        private long GetExpireRemainTime(int selectedCreature)
        {
            ...
            else
            {
              //  if (GameScene.User.IntelligentCreatures[selectedCreature].PetType == GameScene.User.SummonedCreatureType)
             //   {
                    long remainTime = GameScene.User.IntelligentCreatures[selectedCreature].ExpireTime - CMain.Time / 1000;
                    return Math.Max(0, remainTime);
             //   }
             //   else
             //   {
             //       return GameScene.User.IntelligentCreatures[selectedCreature].ExpireTime;
             //   }
            }
        }
    ...
    public sealed class IntelligentCreatureOptionsDialog : MirImageControl
    {
        ...
        public readonly string[] OptionNames = { "所有物品", "金币", "武器", "盔甲", "头盔", "战靴", "腰带", "首饰", "其它" };
    ...
    public sealed class IntelligentCreatureOptionsGradeDialog : MirImageControl
    {
        ...
        private string[] GradeStrings = { "所有", "普通", "稀有", "神话", "传说" };
