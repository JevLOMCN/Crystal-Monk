...
namespace Client.MirControls
{
    public sealed class MirGoodsCell : MirControl
    {
        ...
        private void Update()
        {
            ...
            {
                if (Item.Price() > 1)
                    PriceLabel.Text = string.Format(CMain.Tr("Price: {0} pearls"), (uint)(Item.Price() * GameScene.NPCRate));
                else
                    PriceLabel.Text = string.Format(CMain.Tr("Price: {0} pearl"), (uint)(Item.Price() * GameScene.NPCRate));
            }
            else
            {
                if (Recipe)
                {
                    PriceLabel.Text = string.Format(CMain.Tr("Craftable Item"));
                }
                else
                {
                    PriceLabel.Text = string.Format(CMain.Tr("Price: {0} gold"), (uint)(Item.Price() * GameScene.NPCRate));
                }
            }
        }
