        ...
        protected override unsafe void CreateTexture()
        {
            ...
            if (LineSpace > 0)
            {
                ...
                for (int i = 1; i < Text.Length; i++)
                {
                    if (Text[i - 1] == '\n')
                    {
                        lines.Add(Text.Substring(index, i - index - 1));
                        index = i;
                        continue;
                    }

                    Size size = TextRenderer.MeasureText(Text.Substring(index, i - index), Font);
                ...
                lines.Add(Text.Substring(index, Text.Length - index));
            }
                ...
                using (Graphics graphics = Graphics.FromImage(image))
                {
                    ...
                    if (OutLine)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                TextRenderer.DrawText(graphics, lines[i], Font, new Rectangle(1, 0 + i * height, Size.Width, height), OutLineColour, DrawFormat);
                                TextRenderer.DrawText(graphics, lines[i], Font, new Rectangle(0, 1 + i * height, Size.Width, height), OutLineColour, DrawFormat);
                                TextRenderer.DrawText(graphics, lines[i], Font, new Rectangle(2, 1 + i * height, Size.Width, height), OutLineColour, DrawFormat);
                                TextRenderer.DrawText(graphics, lines[i], Font, new Rectangle(1, 2 + i * height, Size.Width, height), OutLineColour, DrawFormat);
                                TextRenderer.DrawText(graphics, lines[i], Font, new Rectangle(1, 1 + i * height, Size.Width, height), ForeColour, DrawFormat);
                            }
                        }
                    }
                    else
