...
namespace Client.MirControls
{
    public sealed class GameShopCell : MirImageControl
    {
        ...
        public void BuyProduct()
        {
            ...
            {
                if (GameScene.Credit >= Item.CreditPrice * Quantity)
                {
                    CreditCost = Item.CreditPrice * Quantity;
                    messageBox = new MirMessageBox(string.Format(CMain.Tr("Are you sure would you like to buy {1} x {0}({3}) for {2} Credits?"), Item.Info.FriendlyName, Quantity, CreditCost, Item.Count), MirMessageBoxButtons.YesNo);
                }
                else
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(CMain.Tr("You can't afford the selected item."), ChatType.System);
                    return;
                }
            }
            else
            { //Needs to attempt to pay with gold and credits
                if (Item.GoldPrice > 0 && GameScene.Gold >= Item.GoldPrice * Quantity)
                {
                    GoldCost = Item.GoldPrice * Quantity;
                    messageBox = new MirMessageBox(string.Format(CMain.Tr("Are you sure would you like to buy {1} x {0}({3}) for {2} Gold?"), Item.Info.FriendlyName, Quantity, GoldCost, Item.Count), MirMessageBoxButtons.YesNo);
                }
                else
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(CMain.Tr("You can't afford the selected item."), ChatType.System);
        ...
        {
            nameLabel.Text = (Item.Info.Type == ItemType.Pets && Item.Info.Shape == 26 && Item.Info.Effect != 7) ? CMain.Tr("WonderDrug") : Item.Info.FriendlyName;
            nameLabel.Text = nameLabel.Text.Length > 17 ? nameLabel.Text.Substring(0, 17) : nameLabel.Text;
            nameLabel.ForeColour = GameScene.GradeNameColor(Item.Info.Grade);
            quantity.Text = Quantity.ToString();
            goldLabel.Text = Item.GoldPrice > 0 ? (Item.GoldPrice * Quantity).ToString("###,###,##0") : "";
            gpLabel.Text = Item.CreditPrice > 0 ? (Item.CreditPrice * Quantity).ToString("###,###,##0") : "";
